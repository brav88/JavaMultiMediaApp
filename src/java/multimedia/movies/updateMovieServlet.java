...
public class updateMovieServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            String sql = "UPDATE Movies SET name='" + name + 
                         "', director='" + director + 
                         "', date='" + date + 
                         "', genre='" + genre + 
                         "', country='" + country + "' ";
            ...
            if (rowsAffected == 1) {
                ...
                out.println("<h1 style='color:red'>Unhandled error updating the movie</h1>");
                RequestDispatcher rd = request.getRequestDispatcher("getMoviesServlet");
                rd.include(request, response);
            }
        } catch (ClassNotFoundException | SQLException e) {
            out.println("<h1 style='color:red'>Unhandled error updating the movie: " + e.getMessage() + "</h1>");
            RequestDispatcher rd = request.getRequestDispatcher("getMoviesServlet");
            rd.include(request, response);
        }
    }
